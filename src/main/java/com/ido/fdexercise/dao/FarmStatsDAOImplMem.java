...
public class FarmStatsDAOImplMem implements FarmStatsDAO {
  ...
  public void insertFarmDailyStat(final FarmDailyStat farmDailyStat) {
    jdbcTemplate.update("insert into stats values(?, PARSEDATETIME(?, 'yyyy-MM-dd'), ?, ?, ?, ?, ?)",
        ...
        farmDailyStat.getPrecipIn(), farmDailyStat.getTempMin(), farmDailyStat.getTempMax(), farmDailyStat.getLat(), farmDailyStat.getLng());
    log.info("Inserted line to DB. ZipCode={}, SeedingDate={}", farmDailyStat.getZipcode(), dateFormatter.format(farmDailyStat.getDate()));
  }
